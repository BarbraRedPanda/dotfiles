...
bssid=$(nmcli -f SSID,BARS device wifi list | awk 'NR==1 {print; next} {if ($1 == "--") $1="Hidden Network"; print}' | sed '1d' | rofi -theme $HOME/.config/rofi/themes/wifi.rasi -dmenu -p " " -lines 10 | awk '{print $1}')
[ -z "$bssid" ] && exit 1
